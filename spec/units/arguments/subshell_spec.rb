...
describe Gitsh::Arguments::Subshell do
  ...
  describe '#==' do
    it 'returns true when the commands are equal' do
      a1 = described_class.new('A', interpreter_factory: double)
      a2 = described_class.new('A', interpreter_factory: double)

      expect(a1).to eq a2
    end

    it 'returns false when the commands are not equal' do
      a = described_class.new('A', interpreter_factory: double)
      b = described_class.new('B', interpreter_factory: double)

      expect(a).not_to eq b
    end

    it 'returns false when the other object has a different class' do
      arg_a = described_class.new('A', interpreter_factory: double)
